...
class TanhNode(object):
    ...
    def forward(self):
        self.tanh_a = np.tanh(self.a.out)
        self.out = self.tanh_a
        self.d_out = np.zeros(self.out.shape)
        return self.out

    def backward(self):
        d_a = (np.ones(self.tanh_a.shape) - self.tanh_a * self.tanh_a) * self.d_out
        self.a.d_out += d_a 
        return self.d_out 
...
class SoftmaxNode(object):
    ...
    def backward(self):
        d_z = self.d_out @ (self.out * np.identity(self.out.shape[0])-self.out.reshape(-1,1) @ self.out.reshape(1,self.out.shape[0]))
        self.z.d_out += d_z 
        return self.d_out 
...
class NLLNode(object):
    ...
    def backward(self):
        ...
        for i in range(d_y_hat.shape[0]):
            if d_y_hat[i] == 1:
                d_y_hat[i] = self.d_out * (-1/self.y_hat.out[i])
            else: 
                d_y_hat[i] = 0 
