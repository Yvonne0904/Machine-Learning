...
class MulticlassClassifier(BaseEstimator, RegressorMixin):
    ...
    def __init__(self, num_hidden_units=10, step_size=.005, init_param_scale=0.01, max_num_epochs = 1000, num_class=3):
        ...
        self.L_term = nodes.AffineNode(W=self.W1, x=self.x, b=self.b1, node_name='L_term')
        self.h_term = nodes.TanhNode(a=self.L_term, node_name='h_term')
        self.f_term = nodes.AffineNode(W=self.W2, x=self.h_term, b=self.b2, node_name='f_term')
        self.prediction  = nodes.SoftmaxNode(z = self.f_term, node_name='softmax')
 
        self.objective = nodes.NLLNode(y_hat=self.prediction, y_true=self.y, node_name='NLL loss')
